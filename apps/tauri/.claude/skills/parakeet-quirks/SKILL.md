...
- **Splits novel compound brand names.** "OpenWhisper" → "Open Whisper". The tokenizer has no prior on uncommon compounds. Fix: custom vocab post-processing (TASK-10).
- **Phonetic ambiguity on word endings, high confidence not a tell.** Synthesized "Engine" → "Engineer" at 0.96 confidence. High confidence ≠ correct on near-neighbors. Fix: TASK-10 vocab.
...
- **Drops the unstressed copula "er"** in fast Danish speech ("det er helt fint" → "det helt fint"). Same family as v2's a/the drops. Fix: DA rule under TASK-10 — insert "er" in [pronoun + adj/noun] context.
...
1. Check this list first. If it matches → the work goes to TASK-10 (custom vocab / DA rules), not to `core/src/audio.rs` or the recognizer boundary.
